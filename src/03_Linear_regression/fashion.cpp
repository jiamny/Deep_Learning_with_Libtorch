...
FASHION::FASHION(const std::string& root, Mode mode) : mode_(mode) {
...
torch::data::Example<> FASHION::get(size_t index) {
  return {images_[index], targets_[index]};
}

torch::optional<size_t> FASHION::size() const {
  return images_.size(0);
}

bool FASHION::is_train() const noexcept {
  return images_.size(0) == kTrainSize;
}

const torch::Tensor& FASHION::images() const {
  return images_;
}

const torch::Tensor& FASHION::targets() const {
  return targets_;
}
