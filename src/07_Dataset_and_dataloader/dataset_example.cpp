...
struct SensorData {
	...
	SensorData(const std::string &csv_line) {
		...
		while (!iss.eof()) {
			sensor_data.push_back(
					parse_token<decltype(sensor_data)::value_type>(iss));
		}
	}
	std::string row_id;
	int series_id;
	...
	torch::Tensor toTensor() const {
		return torch::tensor(
				torch::ArrayRef<float>(sensor_data.data(), sensor_data.size())).clone();
	}
};
	...
	torch::Tensor toTensor(const std::vector<std::string> &surfaces) const {
		...
		if (iter == surfaces.end()) {
			throw std::logic_error(
					"the surfaces must contain the FloorType::surface");
		}
		long id = iter - surfaces.begin();
		return torch::one_hot(
				torch::tensor( { id }, torch::TensorOptions(torch::kLong)),
				surfaces.size());
	};
...
std::ostream& operator<<(std::ostream &o, const SensorData &d) {
	o << "row_id: " << d.row_id << ", series_id: " << d.series_id
			<< ", measurement_number: " << d.measurement_number
			<< ", sensor_data: " << d.sensor_data << std::endl;
	return o;
}

class CustomDataset: torch::data::Dataset<CustomDataset> {
public:
	CustomDataset(const std::string &x_train_csv,
			const std::string &y_train_csv) {
...
int main(int argc, char *argv[]) {
	...
	std::string x_path = "./data/X_train_sample.csv";
	std::string y_path = "./data/y_train_sample.csv";

	auto x_train_raw = load_csv_file<SensorData>(x_path);
	auto y_train_raw = load_csv_file<FloorType>(y_path);
	...
	for_each(y_train_raw.cbegin(), y_train_raw.cend(),
			[&floor_types](const FloorType &d) {
				if (std::find(floor_types.cbegin(), floor_types.cend(),
						d.surface) == floor_types.end()) {
	...
	std::cout << y_train_raw.at(0).surface << ": "
			<< y_train_raw.at(0).toTensor(floor_types) << std::endl;
/*
	std::cout << y_train_raw.at(1).surface << ": "
			<< y_train_raw.at(1).toTensor(floor_types) << std::endl;
	std::cout << y_train_raw.at(4).surface << ": "
			<< y_train_raw.at(4).toTensor(floor_types) << std::endl;
*/
	std::cout <<"Done!\n";
	return 0;
}
