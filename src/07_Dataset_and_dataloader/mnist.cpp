...
uint32_t expect_int32(std::ifstream &stream, uint32_t expected) {
	...
	// clang-format off
	TORCH_CHECK(value == expected, "Expected to read number ", expected,
...
torch::Tensor read_images(const std::string &root, bool train) {
	const auto path = join_paths(root,
			train ? kTrainImagesFilename : kTestImagesFilename);
	...
	auto tensor = torch::empty( { count, 1, kImageRows, kImageColumns },
			torch::kByte);
...
torch::Tensor read_targets(const std::string &root, bool train) {
	const auto path = join_paths(root,
			train ? kTrainTargetsFilename : kTestTargetsFilename);
