...
int main() {
	...
	std::string dataroot = "/media/hhj/localssd/DL_data/17_flowers/train";
    ...
    std::string valid_dataroot = "/media/hhj/localssd/DL_data/17_flowers/valid";
    ...
    GoogLeNet model(class_num, true, false, true);
	...
	torch::Tensor x = torch::randn({1,3,224,224}).to(device);
	GoogLeNetOutput y = model->forward(x);
	...
	for (epoch = start_epoch; epoch <= total_epoch; epoch++) {
		...
		while (dataloader(mini_batch)) {
			...
			image = std::get<0>(mini_batch).to(device);
			label = std::get<1>(mini_batch).to(device);
			GoogLeNetOutput output = model->forward(image);
			auto out = torch::nn::functional::log_softmax(output.output, 1); // dim
		...
		if( valid && (epoch % 5 == 0) ) {
			...
			while (valid_dataloader(mini_batch)){
			    ...
			    responses = output.output.exp().argmax(1); // dim
	...
	if( test ) {
		std::cout << "--------------- Testing --------------------\n";
		std::string test_dataroot = "/media/hhj/localssd/DL_data/17_flowers/test";
	    ...
	    while( test_dataloader(data) ){
	        ...
	        GoogLeNetOutput output = model->forward(image);
	        auto out = torch::nn::functional::log_softmax(output.output, 1);
	        ...
	        int64_t response = opt.argmax(1).item<int64_t>();
