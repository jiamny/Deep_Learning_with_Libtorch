...
int main() {
	...
	GoogLeNet net = GoogLeNet(10, true, false, true);

	auto dict16bn = net->named_parameters();
	for (auto n = dict16bn.begin(); n != dict16bn.end(); n++) {
	    std::cout<<(*n).key()<<std::endl;
	}

	torch::Tensor test_x = torch::zeros({1, 3, 96, 96});
	GoogLeNetOutput test_y = net(test_x);
	std::cout << test_y.output << std::endl;
    ...
    std::string model_file_name="./models/custom_googlenet.pt";
    ...
    auto train_set = dataSetClc(train_data_path, ".png", 96, classes);
    ...
    auto test_set = dataSetClc(test_data_path, ".png", 96, cls, classes.size());
...
// std::unique_ptr<StatelessDataLoader<Dataset, Sampler>>>
    ...
    // (5) Define Network
	GoogLeNet model = GoogLeNet(classes.size(), true, false, true);
    model->to(device);
...
//            std::cout << labels << std::endl;
            ...
            GoogLeNetOutput outputs = model->forward(inputs);
            auto loss = criterion(outputs.output, labels);
            loss.backward();
            optimizer.step();
    ...
    if( saveModel ) {
    	...
    	model = GoogLeNet(10, true, false, true);
    ...
    for (const auto& batch : *test_data_loader) {
        ...
        auto out_tuple = torch::max(outputs.output, 1);
