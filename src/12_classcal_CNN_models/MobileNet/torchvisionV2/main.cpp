...
int main() {
     ...
     const size_t imgSize{64};
     const double  width_mult{1.0};
     const int64_t num_classes{10};
     const std::vector<std::vector<int64_t>> inverted_residual_settings = {};
     const int64_t round_nearest{8};
     ...
     MobileNetV2 net( num_classes, width_mult, inverted_residual_settings, round_nearest );
     net->to(device);
     ...
     torch::Tensor x = torch::randn({1, 3, 64, 64});
     torch::Tensor y = net(x);
     std::cout << y << std::endl;
     std::cout << device << std::endl;

   	 std::string model_dir="./models";
   	 std::string model_file_name="./models/torchvisionV2_mobilenet.pt";
   	...
   	auto test_loader = torch::data::make_data_loader(
   	        std::move(test_set_transformed), torch::data::DataLoaderOptions()
   	                                             ...
   	                                             .enforce_ordering(true));
    // Model
   	MobileNetV2 model( num_classes, width_mult, inverted_residual_settings, round_nearest );
    model->to(device);
    ...
    if( saveModel ) {
        ...
        model = MobileNetV2( num_classes, width_mult, inverted_residual_settings, round_nearest );
