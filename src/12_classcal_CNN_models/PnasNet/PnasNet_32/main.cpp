...
int main() {
	std::cout << "Test pnasnet...\n\n";

	PNASNet net = PNASNetB("CellB", 6, 32, 10);
    net->to(torch::kCPU);
    ...
    net = PNASNetB("CellB", 6, 32, classes.size());
    net->to(torch::kCPU);
    ...
    const double weight_decay{5e-4};

    //torch::optim::SGD optimizer(net->parameters(), torch::optim::SGDOptions(0.001).momentum(0.9));
    auto optimizer = torch::optim::SGD(net->parameters(), torch::optim::SGDOptions(lr).momentum(momentum).weight_decay(weight_decay));

    //auto scheduler = torch::optim::LRScheduler(optimizer);
    // Train the network
    ...
    if( saveModel ) {
    	std::string PATH = "./models/pnasnet_32.pth";
    	torch::save(net, PATH);
