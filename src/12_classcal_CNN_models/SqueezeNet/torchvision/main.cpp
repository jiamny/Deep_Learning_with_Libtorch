...
int main() {
     ...
     SqueezeNet net(version, num_classes);
     net->to(device);
     ...
     std::cout << device << std::endl;
   	 ...
   	 std::string model_file_name="./models/torchvision_squeezenet.pt";
   	...
   	auto test_loader = torch::data::make_data_loader(
   	        std::move(test_set_transformed), torch::data::DataLoaderOptions()
   	                                             ...
   	                                             .enforce_ordering(true));
    // Model
   	SqueezeNet model(version, classes.size());
    model->to(device);
    ...
    if( saveModel ) {
        ...
        model = SqueezeNet(version, classes.size());
