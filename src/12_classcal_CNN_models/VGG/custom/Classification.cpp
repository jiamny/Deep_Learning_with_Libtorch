...
void Classifier::Train(int num_epochs, int batch_size, int image_size, float learning_rate,
						std::string train_val_dir, std::string image_type, std::string save_path){
    std::string path_train = train_val_dir+ "/train";
    std::string path_val = train_val_dir + "/val";

    auto custom_dataset_train = dataSetClc(path_train, image_type, image_size).map(torch::data::transforms::Stack<>());
    auto custom_dataset_val = dataSetClc(path_val, image_type, image_size).map(torch::data::transforms::Stack<>());
    ...
    for (size_t epoch = 1; epoch <= num_epochs; ++epoch) {
        ...
        for (auto& batch : *data_loader_train) {
            ...
            target = target.to(torch::kInt64).to(device).flatten(0, -1); // flatten to 1d target
            optimizer.zero_grad();
            // Execute the model
