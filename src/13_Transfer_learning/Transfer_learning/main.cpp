...
torch::Tensor read_data(std::string location) {
    ...
    */
    ...
    cv::Mat img = cv::imread(location.c_str(), 1);
    if(! img.empty() ) {
        ...
        if((dir = opendir(base_name.c_str())) != NULL) {
            while((ent = readdir(dir)) != NULL) {
                ...
                if(filename.length() > 4 && filename.substr(filename.length() - 3) == "jpg") {
                    //std::cout << base_name << " <=> " << ent->d_name << std::endl;
                    //std::cout << "Add /=> " << base_name + "/" + ent->d_name << std::endl;
                	// Add /=> /root/PycharmProjects/图像_datasets/cat_dog/train/Dog//10307.jpg
                	// --- Exclude empty image
