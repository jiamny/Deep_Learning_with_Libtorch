...
// ---------------------------------------------------
WAE_EncoderImpl::WAE_EncoderImpl(size_t nf, int size, int nc, size_t nz){
    
    size_t feature = nf;
    size_t ns = size / (size_t)(std::pow(2, 6));

    DownSampling(this->model, nc, feature, /*BN=*/false, /*ReLU=*/true);  					 // {C,256,256} ===> {F,128,128}
    ...
    this->model->push_back(nn::Linear(feature*8*ns*ns, nz));              					 // {8F*4*4}    ===> {Z}
...
// ---------------------------------------------------
WAE_DecoderImpl::WAE_DecoderImpl(size_t nf, int size, int nc, size_t nz){
    
    size_t feature = nf;
    size_t ns = size / (size_t)(std::pow(2, 6));

    this->model->push_back(nn::Linear(nz, feature*8*ns*ns));                 					// {Z}         ===> {8F*4*4}
    ...
    UpSampling(this->model, feature, nc, /*BN=*/false, /*ReLU=*/false);      					// {F,128,128} ===> {C,256,256}
...
// ---------------------------------------------------------
GAN_DiscriminatorImpl::GAN_DiscriminatorImpl(size_t nz, size_t nd, size_t n_blocks){
    
    this->model->push_back(nn::Linear(nz, nd));
    this->model->push_back(nn::ReLU(nn::ReLUOptions().inplace(true)));
    for (size_t i = 0; i < n_blocks; i++){
        this->model->push_back(nn::Linear(nd, nd));
        this->model->push_back(nn::ReLU(nn::ReLUOptions().inplace(true)));
    }
    this->model->push_back(nn::Linear(nd, 1));
...
// ---------------------------------------------------------------
torch::Tensor GAN_DiscriminatorImpl::forward(torch::Tensor z){
    torch::Tensor out = this->model->forward(z);  // {Z} ===> {1}
    return out;
}
