...
// ----------------------------------------------------------------------
ConvolutionalAutoEncoderImpl::ConvolutionalAutoEncoderImpl(size_t nf, size_t nc, size_t nz){
    
    size_t feature = nf;

    DownSampling(this->encoder, nc, feature, /*BN=*/false, /*ReLU=*/true);     					  // {C,256,256} ===> {F,128,128}
    ...
    DownSampling(this->encoder, feature*8, nz, /*BN=*/false, /*ReLU=*/false);  					  // {8F,8,8}    ===> {Z,4,4}
    register_module("encoder", this->encoder);

    UpSampling(this->decoder, nz, feature*8, /*BN=*/true, /*ReLU=*/true);      					  // {Z,4,4}     ===> {8F,8,8}
    ...
    UpSampling(this->decoder, feature, nc, /*BN=*/false, /*ReLU=*/false);      					  // {F,128,128} ===> {C,256,256}
