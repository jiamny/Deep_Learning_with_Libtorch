...
void Detector::loadPretrained(std::string pretrained_pth) {
	 ...
	 *
	 *  Save model on CPU, Load model on GPU : torch::load(PATH, "cuda:0") // device = torch::Device( torch::kCUDA, gpu_id )
	 */
	if( torch::cuda::is_available() ) {
		// Save model on GPU, Load model on GPU
		torch::load(net_pretrained, pretrained_pth);
		net_pretrained->to(device);
	} else {
		// Save model on GPU, Load model on CPU
		torch::load(net_pretrained, pretrained_pth, torch::Device(torch::kCPU));
	}
...
void Detector::LoadWeight(std::string weight_path) {
	/*
	 *  bug fix for error: Could not run 'aten::empty_strided' with arguments from the 'CUDA' backend.
	 *
	 *  weights saved on GPU, load on CPU - torch::load(detector, weight_path, torch::Device(torch::kCPU));
	 *  weights saved on GPU, load on GPU -
