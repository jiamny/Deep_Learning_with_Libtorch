...
int main() {
	...
	std::string video_path = "./data/videoSample2.mp4";
    // Loading  Module
    torch::jit::script::Module module = torch::jit::load("./src/19_ObjectDetection/YOLOv5/yolov5s.torchscript.pt");

    std::vector<std::string> classnames;
	std::ifstream f("./src/19_ObjectDetection/YOLOv5/coco.names");
	std::string name = "";
	while (std::getline(f, name)) {
		classnames.push_back(name);
	}

    cv:: VideoCapture cap = cv::VideoCapture(video_path.c_str());

    if( !cap.isOpened() ) {
    	std::cout << "Couldn't find " << video_path << std::endl;
    	return 1;
    }
    ...
    while(cap.isOpened()) {
	    ...
	    if(frame.empty()) {
           std::cout << "Read frame failed or at the end of the video!" << std::endl;
           break;
        }
