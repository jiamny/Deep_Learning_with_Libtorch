...
Parameter::Parameter()
{
	...
	this->datas.insert(std::make_pair(INPUT, Desc("--input", "-i", std::string("./data/bus.jpg"), INT | STRING, STRING,
	...
	this->datas.insert(std::make_pair(DEVICE, Desc("--device", "-d", std::string("cuda:0"), STRING, STRING, "cuda device, i.e. cuda:0 or cuda:1,cuda:2 or cpu")));
	...
	this->datas.insert(std::make_pair(LABEL_PATH, Desc("--label_path", "-lp", std::string("./src/19_ObjectDetection/Yolo_libtorch/resource/coco.txt"),
														STRING, STRING, "label path.")));
	...
	this->datas.insert(std::make_pair(IS_IMAGE, Desc("--is_image", "-ii", true, BOOL, BOOL, "an image.")));
	...
	this->datas.insert(std::make_pair(MODEL_PATH, Desc("--model_path", "-mp", std::string("./src/19_ObjectDetection/Yolo_libtorch/resource/yolov8n.cuda.torchscript"),
