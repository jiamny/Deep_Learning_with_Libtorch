...
int main() {
	...
	std::string filename = "./data/camera.yml";
	...
	if (exists) {
		...
		std::string path_to_files = "./data/camera_cal";
	...
	cv::Mat sample_img = cv::imread("./data/faulty.jpg");
	//imshow("faulty", sample_img);
	cv::Mat un = calibrator.remap(sample_img);

	cv::VideoCapture cap("/media/hhj/localssd/DL_data/videos/challenge_video.mp4");
	cv::VideoWriter video("./data/outcpp.avi", cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 25, un.size());
	...
	while (1) {
		...
		// display image
	    namedWindow( "AdvancedLaneDetection", cv::WINDOW_AUTOSIZE );
	    imshow( "", warp_back );
	    cv::waitKey(25);
