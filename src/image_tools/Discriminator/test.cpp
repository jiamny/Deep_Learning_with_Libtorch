...
void test(Option_Arguments &vm, torch::Device &device, MC_Discriminator &model, std::vector<transforms_Compose> &transform, const std::vector<std::string> class_names){
    ...
    // (1) Get Test Dataset
    dataroot = vm.dataroot + "/" + vm.dataset + "/" + vm.test_dir;
    ...
    // (2) Get Model
    path = "checkpoints/" + vm.dataset + "/models/epoch_" + vm.test_load_epoch + ".pth";
    ...
    result_dir = vm.test_result_dir;  fs::create_directories(result_dir);
