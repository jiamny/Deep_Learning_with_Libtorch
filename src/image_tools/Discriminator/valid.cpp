...
void valid(Option_Arguments &vm, DataLoader::ImageFolderClassesWithPaths &valid_dataloader, torch::Device &device, Loss &criterion, MC_Discriminator &model, const std::vector<std::string> class_names, const size_t epoch){
    ...
    ofs.open("checkpoints/" + vm.dataset + "/log/valid.txt", std::ios::app);
    ofs << "epoch:" << epoch << '/' << vm.epochs << ' ' << std::flush;
    ofs << "classify:" << ave_loss << ' ' << std::flush;
    ofs << "accuracy:" << total_accuracy << std::endl;
    ofs.close();

    // (5.2) Record Loss (Log/Accuracy)
    ofs.open("checkpoints/" + vm.dataset + "/log/valid.csv", std::ios::app);
    if(epoch == 1){
        ofs << "epoch," << std::flush;
