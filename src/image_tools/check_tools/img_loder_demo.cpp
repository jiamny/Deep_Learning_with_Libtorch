...
struct AlexNetImpl : torch::nn::Module {
    ...
    AlexNetImpl(int64_t class_num)
            ...
            linear1(register_module("linear1", torch::nn::Linear(256*6*6, 4096))),
            linear2(register_module("linear2", torch::nn::Linear(4096, 4096))),
            linear3(register_module("linear3", torch::nn::Linear(4096, class_num))),
...
/*
    ...
    auto tensor = t4mat.reshape({width * height * t4mat.size(2)});
    cv::Mat mgm(cv::Size{ width, height }, CV_8UC3, tensor.data_ptr<uchar>());
